...
function mediumUrl(squareUrl: string): string {
  return squareUrl.replace(/\/square\b/, "/medium").replace(/square\./, "medium.")
}

function displayName(s: Sighting): string {
  if (!s.commonName) return s.speciesName
  return s.commonName.charAt(0).toUpperCase() + s.commonName.slice(1)
}
...
function SightingRow({
  ...
  const name = displayName(sighting)
  const italic = sighting.commonName ? sighting.speciesName : null
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {italic && (
          <p className="text-xs italic text-muted-foreground truncate">{italic}</p>
        )}
        {sighting.placeGuess && (
    ...
    <>
      <Card variant="default">
        <CardBody className="py-1 px-4">
          {sightings.map((s) => (
            <SightingRow key={s.id} sighting={s} onClick={() => setSelected(s)} />
          ))}
        </CardBody>
      </Card>
            ...
            <>
              ...
              <div className="p-5">
                <ModalHeader className="mb-3">
                  <ModalTitle>{displayName(selected)}</ModalTitle>
                  {selected.commonName && (
                    <ModalDescription className="italic">
