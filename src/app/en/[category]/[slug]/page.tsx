...
export async function generateStaticParams() {
  ...
  const { data } = await supabase
    ...
    .eq("status", "published");
  return (data ?? []).map((a) => ({
    category: toCategorySlug(a.category ?? "articles"),
    slug: a.slug,
  }));
}
...
export async function generateMetadata({
  ...
  const canonicalUrl = `https://outdoorpatagonia.com/en/${catSlug}/${slug}`;
  ...
  return {
    ...
    alternates: {
      ...
      ...(altLang && {
        languages: {
          es: `https://outdoorpatagonia.com/${toCategorySlug(altLang.category ?? "")}/${slug}`,
        },
      }),
    },
    openGraph: {
...
export default async function ArticlePage({
  ...
  if (category !== correctCategory) {
    redirect(`/en/${correctCategory}/${slug}`);
  }

  const altLangHref = altLang
    ? `/${toCategorySlug(altLang.category ?? "")}/${slug}`
    : null;
  ...
  const jsonLd = {
    ...
    url: `https://outdoorpatagonia.com/en/${correctCategory}/${slug}`,
  };
